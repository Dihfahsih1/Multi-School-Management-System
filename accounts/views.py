...
def addstudent(request):
   if request.method=="POST":
       form=AddBasicInfoForm(request.POST,request.FILES)
       if form.is_valid():
           form.save()
           return redirect('addcontactstudent')
   else:
        form = AddBasicInfoForm()
        context = {'form': form}
        return render(request, 'accounts/Student/addstudent.html', context)

def addcontactstudent(request):
   if request.method=="POST":
       form=AddContactInfoForm(request.POST,request.FILES)
       if form.is_valid():
           form.save()
           return redirect('addacademicstudent')
   else:
        form = AddContactInfoForm()
        context = {'form': form}
        return render(request, 'accounts/Student/addstudentcontact.html', context)
def addacademicstudent(request):
   if request.method=="POST":
       form=AddAcademicInfoForm(request.POST,request.FILES)
       if form.is_valid():
           form.save()
           return redirect('addstudent')
   else:
        form = AddAcademicInfoForm()
        context = {'form': form}
        return render(request, 'accounts/Student/addstudentacademic.html', context)

def editstudent(request, pk):
   item = get_object_or_404(Student, id=pk)
   if request.method=="POST":
       form=EditBasicInfoForm(request.POST,request.FILES, instance=item)
       if form.is_valid():
           form.save()
           return redirect('viewstudent')
   else:
        form = EditBasicInfoForm(instance=item)
        context = {'form': form}
        return render(request, 'accounts/Student/editstudent.html', context)

def deletestudent(request, pk):
   Student.objects.filter(id=pk).delete()
   all_info=Student.objects.all()
   context={'all_info' :all_info}
   return render(request, 'accounts/Student/viewstudent.html', context)
